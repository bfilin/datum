...
def basic_demo():
    ...
    # example 3 
    header('[ example 3: get hash of content of ' + my_file_path_and_name + ', useful for generating digest of file for this project]')
    ...
    h1.update(b'dir_a/dir_b2/file_b2.txt')
    h1.update(text)
    x1 = h1.digest()


    h2.update(b'dir_a/dir_b2/file_b2.txt' + text)
    x2 = h2.digest()


    h3.update(b'dir_a/dir_b2/file_b2.txt')
    x3 = h1.digest()
    h3.update(text)
    x4 = h3.digest()

    Yields the following results:
    hexlify(x1) == hexlify(x2) == hexlify(x4)
